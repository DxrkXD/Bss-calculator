...
# ==========================================================
APP_NAME = "BSS Calculator"
APP_VERSION = "1.0.4"  # <-- bump this when you release
# Host a tiny JSON file somewhere public (GitHub raw, pastebin raw, your website) like:
# {"latest":"1.0.4","download_url":"https://example.com/BSSCalculator.exe","notes":"What's new..."}
UPDATE_JSON_URL = "https://example.com/bss_calculator_update.json"
a
# ==========================================================
# Honey parse/format
# ==========================================================
...
KNOWN_COSTS = {
    1: 1_000, 2: 5_000, 3: 20_000, 4: 80_000, 5: 300_000,
    6: 900_000, 7: 2_500_000, 8: 6_000_000, 9: 15_000_000, 10: 35_000_000,
    11: 80_000_000, 12: 150_000_000, 13: 300_000_000, 14: 600_000_000,
    15: 150_000_000_000, 16: 400_000_000_000, 17: 1_200_000_000_000,
    18: 15_000_000_000_000, 19: 75_000_000_000_000, 20: 250_000_000_000_000,
}

def projected_cost(level: int) -> int:
    base = KNOWN_COSTS[20]
    growth = 2.15
    return int(base * (growth ** (level - 20)))

def cost_per_bee(level: int) -> int:
    return KNOWN_COSTS.get(level, projected_cost(level))
...
root = tk.Tk()
root.title(f"BSS Calculator v{APP_VERSION}")
...
def _parse_version(v: str):
    """
    Very small version parser: '1.0.4' -> (1,0,4)
    Non-numeric parts are ignored.
    """
    nums = re.findall(r"\d+", str(v))
    return tuple(int(x) for x in nums[:4])  # up to 4 parts

def _fetch_update_json() -> dict:
    with urllib.request.urlopen(UPDATE_JSON_URL, timeout=4) as resp:
        data = resp.read().decode("utf-8", errors="replace")
    return json.loads(data)

def check_for_updates(manual: bool = False):
    """
    Checks UPDATE_JSON_URL for a JSON containing at least {"latest":"x.y.z"}.
    Runs in a thread so the UI doesn't freeze.
    """
    if not UPDATE_JSON_URL or "example.com" in UPDATE_JSON_URL:
        if manual:
            messagebox.showinfo(
                "Update Check",
                "Update checking is enabled, but UPDATE_JSON_URL is still the placeholder.\n\n"
                "To use it:\n"
                "1) Upload a small JSON file online (GitHub raw works great)\n"
                '2) Set UPDATE_JSON_URL to the raw JSON link\n\n'
                'Example JSON:\n{"latest":"1.0.4","download_url":"https://.../bss_hive_calculator.exe","notes":"..."}'
            )
        return

    def worker():
        try:
            info = _fetch_update_json()
            latest = str(info.get("latest", "")).strip()
            dl = str(info.get("download_url", "")).strip()
            notes = str(info.get("notes", "")).strip()

            if not latest:
                raise ValueError("Update JSON missing 'latest' field.")

            cur_v = _parse_version(APP_VERSION)
            new_v = _parse_version(latest)
            ...
            def ui_latest():
                if new_v > cur_v:
                    ...
                    if manual:
                        messagebox.showinfo("Up to date", f"You're up to date.\n\nCurrent: v{APP_VERSION}")
                set_status("Update check finished")

            root.after(0, ui_latest)

        except Exception as e:
            def ui_err():
                if manual:
                    messagebox.showwarning("Update Check Failed", f"Couldn't check for updates.\n\n{e}")
...
def init():
    ...
    apply_theme()
    set_status("Ready")
    # Auto-check for updates on startup (silent on errors)
